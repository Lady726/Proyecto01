...
public class AccesoAleatorio {
    ...
    public static boolean setPersona(int i, Persona persona) throws IOException {
        if(i >= 0 && i <= getNumeroRegistros()) {
            if(persona.getTamaño() > tamañoRegistro) {
                ...
                flujo.writeUTF(persona.getApellidos());
                flujo.writeUTF(persona.getDir());
                flujo.writeInt(persona.getId());
                flujo.writeInt(persona.getSalario());
                flujo.writeInt(persona.getTelefono());
    ...
    public static void compactarArchivo(File archivo) throws IOException {
        ...
        File tempo = new File("empleados.dat");
    ...
    public static Persona getPersona(int i) throws IOException {
        if(i >= 0 && i <= getNumeroRegistros()) {
            flujo.seek(i * tamañoRegistro);
            return new Persona(flujo.readUTF(), flujo.readUTF(), flujo.readUTF(), flujo.readInt(), flujo.readInt(),flujo.readInt(),flujo.readBoolean());
        } else {
            System.out.println("\nNúmero de registro fuera de límites.");
            return null;
        }
